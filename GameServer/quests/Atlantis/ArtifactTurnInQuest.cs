	...
	public class ArtifactTurnInQuest : ArtifactQuest
	{
		...
		public override bool WhisperReceive(GameLiving source, GameLiving target, string text)
		{
			...
			if (m_curTypes.Contains(text))
			{
				...
				Dictionary<String, DbItemTemplate> versions = ArtifactMgr.GetArtifactVersions(ArtifactID, (eCharacterClass)player.CharacterClass.ID, (eRealm)player.Realm);
				...
				else
				{
					...
					DbItemTemplate template = versions[m_chosenTypes] as DbItemTemplate;

					if (GiveItem(player, template))
					{
						FinishQuest();
						scholar.SayTo(player, eChatLoc.CL_PopupWindow, string.Format("Here is your {0}, {1}. May it serve you well!", ArtifactID, player.Name));
						return true;
					}

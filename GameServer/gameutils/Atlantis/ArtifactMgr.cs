...
namespace DOL.GS
{
	...
	public sealed class ArtifactMgr
	{
		...
		public static Dictionary<String, DbItemTemplate> GetArtifactVersions(String artifactID, eCharacterClass charClass, eRealm realm)
		{
			...
			List<DbArtifactXItem> allVersions = GetArtifactVersions(artifactID, realm);
			Dictionary<String, DbItemTemplate> classVersions = new Dictionary<String, DbItemTemplate>();

			lock (allVersions)
			{
                DbItemTemplate itemTemplate;
				foreach (DbArtifactXItem version in allVersions)
				{
					itemTemplate = GameServer.Database.FindObjectByKey<DbItemTemplate>(version.ItemID);

				...
				lock (QuestListToGive.SyncRoot)
				{
					...
					foreach (ArtifactTurnInQuest quest in QuestListToGive)
					{
						...
						foreach (string art in arts)
						{
							...
							if (versions.Count > 1)
							{
								this.SayTo(player, string.Format("{0}, I see that you carry {1}. If you are interested, I will exchange yours for you but there is a price to pay for the change. Any experience or levels your artifact has gained will be lost when the change occurs. Are you still [interested]?", player.Name, art));
							}
						}
						return true;
					}
